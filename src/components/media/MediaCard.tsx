...
export default function MediaCard({
  ...
  const showCover = !coverFailed && hasRenderableBookCover(item.image);
  const shellClass = cn(
    ...
    <>
      ...
      >
        {showCover ? (
          ...
          <div
            className={cn(
              'flex h-full flex-col justify-end p-3 text-sm',
              LIBRARY_MUTED
            )}
          >
            <div className={cn('line-clamp-3 font-medium leading-5 text-library-ink dark:text-library-night-ink', LIBRARY_SERIF)}>
              {item.title}
            </div>
            {item.subtitle ? (
              <div className='mt-1 line-clamp-1 text-[11px]'>{item.subtitle}</div>
            ) : null}
          </div>
        )}
