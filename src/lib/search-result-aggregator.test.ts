...
describe('groupSearchResults', () => {
  ...
  it('groups exact-search hits after the query filter is applied', () => {
    const hits = [
      result({ id: '1', title: '某剧4', source: '源A' }),
      result({ id: '2', title: '某剧第四季', source: '源B' }),
      result({ id: '3', title: '另一剧', source: '源C' }),
    ].filter((item) => item.title.includes('某剧'));
    const grouped = groupSearchResults(hits);
    expect(grouped).toHaveLength(1);
    expect(grouped[0][1]).toHaveLength(2);
  });
});
