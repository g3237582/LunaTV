...
import { nextPrefetchPages, parsePageParam } from '@/lib/music-page-data';
    ...
    fetch(`/api/music/v2/discovery/songlists?source=${source}&tagId=${encodeURIComponent(tagId)}&sortId=${encodeURIComponent(sortId)}&page=${page}`)
      ...
      .then((data) => {
        if (data.success) {
          ...
          setTotal(next.total);
          writeCache(cacheKey, next);
          const pageSize = Math.max(next.list.length || 1, 1);
          const pageCount = Math.max(1, Math.ceil((next.total || 0) / pageSize));
