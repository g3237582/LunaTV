...
export default function MangaRecommendPage() {
  ...
  useEffect(() => {
    void loadInParallel({
      ...
      history: () => getAllMangaReadRecords(),
    })
      .then((loaded) => {
        const nextSources =
          (loaded.sources as { sources?: MangaSource[] } | null)?.sources || [];
        setSources(nextSources);
        setSourceId((prev) => prev || nextSources[0]?.id || '');
        if (loaded.shelf) setShelf(loaded.shelf);
        if (loaded.history) {
          setReadRecords(
            Object.values(loaded.history).sort((a, b) => b.saveTime - a.saveTime)
          );
        }
      })
