...
function SearchPageClient() {
  ...
  const filteredAggResults = useMemo(() => {
    ...
    const filtered = aggregatedResults.filter(([_, group]) => {
      ...
      if (title !== 'all' && !group.some((item) => item.title === title))
        return false;
      if (
        year !== 'all' &&
        !group.some((item) => (item.year || 'unknown') === year)
      )
        return false;
      return true;
    });
          ...
          {showResults ? (
            <section className='mb-12'>
              ...
              {searchResults.length === 0 ? (
                ...
                <div key={`search-results-${viewMode}-${safePage}`}>
                  {viewMode === 'agg' ? (
                    <VirtualGrid
                      items={pagedAggResults}
                      ...
                      renderItem={([mapKey, group]) => {
                        const { title, poster, year } = pickGroupDisplay(group);
