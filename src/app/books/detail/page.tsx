  ...
  return (
    <div className='space-y-6'>
      ...
      >
        ...
        >
          {hasRenderableBookCover(detail.cover) && !coverFailed ? (
            <>
              <ProxyImage
            ...
            <div
              className={cn(
                'flex h-full flex-col justify-end gap-1 p-4 text-sm',
                LIBRARY_MUTED
              )}
            >
              <div className={cn('line-clamp-4 text-base font-medium text-library-ink dark:text-library-night-ink', LIBRARY_SERIF)}>
                {detail.title}
              </div>
              {detail.author ? <div className='line-clamp-1 text-xs'>{detail.author}</div> : null}
            </div>
          )}
        </div>
